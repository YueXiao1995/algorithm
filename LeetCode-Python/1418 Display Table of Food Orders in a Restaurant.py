...
def displayTable(orders):
    ...
    for food in food_items:
        initial_food_dict[food] = 0
    print(initial_food_dict)
    ...
    for order in orders:
        ...
        food_item = order[2]
        if table_number not in table_order:
            table_order[table_number] = initial_food_dict.copy()
        table_order[table_number][food_item] += 1

    print(table_order)

    result = list()
    result.append(["Table"]+food_items)
    tables = sorted(table_order.keys())
    print(tables)

    for table in tables:
        row = [str(table)]
        for food in food_items:
            row.append(str(table_order[table][food]))
        result.append(row)
